...
namespace Waffles
{
	...
	class OldAutomaton : Automaton<StateSymbolPair, HashSet<State>>
    {
		public HashSet<State> FinalStates { get; private set; }

		public OldAutomaton(HashSet<State> states, HashSet<Symbol> alphabet, FSATransitionFunction transFunc, State start, HashSet<State> final)
		{
			this.States = states;
        ...
        public OldAutomaton(OldAutomaton automaton)
        {
        ...
        public override OldAutomaton CreateDeterministicAutomaton()
        {
            //return if we're already deterministic.
            if (IsDeterministic())
                return new OldAutomaton(this);
            ...
            return new OldAutomaton(states, Alphabet, map, 
        ...
        public HashSet<State> GetStatesAccessibleFrom(State state, char input, List<StatePair> pastStates=null)
        {
            ...
            else
            {
                ...
                while (pastID >= 0 && pastStates[pastID].Value == Symbols.Epsilon)
                {
            ...
            if (nextEpsilonStates != null)
            {
                ...
                {
                    StatePair currentState = new StatePair(state, Symbols.Epsilon);
